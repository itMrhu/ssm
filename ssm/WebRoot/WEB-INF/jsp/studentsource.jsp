...
<script type="text/javascript">
  	...
  	function setdatagrid(){
  		$('#tt').datagrid({
  			idField:'id' ,		
			fit:true , 					
			url:'${pageContext.request.contextPath}/student/getStudentlist.action' ,
			fitColumns:true ,  				
			striped: true ,					
...
//			singleSelect:true ,		
			...
			columns:[[
				{field:'stuname',title:'学生姓名' ,width:150 ,align:'center' ,sortable:true},
				{field:'ssm',title:'ssm' ,width:150 ,align:'center' ,sortable:true},
				{field:'python',title:'python' ,width:150 ,align:'center' ,sortable:true},
				{field:'java',title:'java' ,width:150 ,align:'center' ,sortable:true},
			...
			toolbar:[
				{
					...
					handler:function(){
						...
						if(arr.length != 1){
							...
							$('#mydialog').dialog('open'); 
							$('#id').val(arr[0].id);
							$('#stuname').val(arr[0].stuname);
							$('#ssm').val(arr[0].ssm);
							$('#python').val(arr[0].python);
							$('#java').val(arr[0].java);
	...
	//查询
	function searchUser(){
		var stuname_se = $('#stuname_se').val();
		var data = {"stuname":stuname_se};
...
//   	提交表单
	function submitmyform(){
		...
		if($('#myform').form('validate')){
			$.ajax({
				...
				cache:false ,
				data:JSON.stringify({id:id,stuname:stuname,ssm:ssm,python:python,java:java,status:status}),  
	...
	function upload(){
		...
		$('#uform').form('submit',{
            url:'${pageContext.request.contextPath }/student/uploadStudent.action',
            onSubmit:function(){
  ...
  <body class="easyui-layout">
		<div region="north"  style="height:30px;" >
			<form id="mysearch" method="post">
					学生姓名:<input id="stuname_se"  name="stuname_se" type="text" style="width:100px;" class="easyui-validatebox" value="" />&nbsp;&nbsp;
		...
		<div id="mydialog" title="新增" modal="true"  draggable="false" class="easyui-dialog" closed="true" style="width:300px;">
	    	<form id="myform" action="" method="post">
	    		...
	    		<table>
	    			<tr>
	    				<td >学生姓名</td>
	    				<td><input id="stuname"  name="stuname" type="text" class="easyui-validatebox" required="true"  value="" /></td>
	    			</tr>
	    			<tr>
	    				<td>ssm</td>
	    				<td><input type="text" name="ssm" id="ssm" class="easyui-validatebox" required="true"   value="" /></td>
	    			</tr>
	    			<tr>
	    				<td>python</td>
	    				<td><input type="text" name="python" id="python" class="easyui-validatebox" required="true"   value="" /></td>
	    			</tr>
	    			<tr>
	    				<td>java</td>
	    				<td><input type="java" name="java" id="java" class="easyui-validatebox" required="true"   value="" /></td>
	    			</tr>
   					<tr>
   						<td>状态</td>
